...
func TestListInputBindings(t *testing.T) {
	...
	require.NoError(t, err)
	resp, err := server.ListInputBindings(t.Context(), connect.NewRequest(&emptypb.Empty{}))
	require.NoError(t, err)
	assert.NotNil(t, resp)
	assert.Lenf(t, resp.Msg.GetBindings(), 2, "expected 2 handlers")
}
...
func TestBinding(t *testing.T) {
	...
	t.Run("binding event for wrong method", func(t *testing.T) {
		in := connect.NewRequest(&runtimev1.BindingEventRequest{Name: "invalid"})
		_, err := server.OnBindingEvent(ctx, in)
		require.Error(t, err)
	})

	t.Run("binding event without data", func(t *testing.T) {
		in := connect.NewRequest(&runtimev1.BindingEventRequest{Name: methodName})
	...
	t.Run("binding event with data", func(t *testing.T) {
		data := "hello there"
		in := connect.NewRequest(&runtimev1.BindingEventRequest{
			Name: methodName,
			Data: []byte(data),
		})
		out, err := server.OnBindingEvent(ctx, in)
		require.NoError(t, err)
		assert.NotNil(t, out)
		assert.Equal(t, data, string(out.Msg.GetData()))
	})
