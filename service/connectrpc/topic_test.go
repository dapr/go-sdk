...
func TestTopicSubscriptionList(t *testing.T) {
	...
	resp, err := server.ListTopicSubscriptions(t.Context(), connect.NewRequest(&emptypb.Empty{}))
	require.NoError(t, err)
	assert.NotNil(t, resp)
	if assert.Lenf(t, resp.Msg.GetSubscriptions(), 1, "expected 1 handlers") {
		sub := resp.Msg.GetSubscriptions()[0]
...
func TestTopicWithErrors(t *testing.T) {
	...
	t.Run("topic event for retry error", func(t *testing.T) {
		...
		require.Error(t, err)
		assert.Equal(t, runtime.TopicEventResponse_RETRY, resp.Msg.GetStatus())
	})
