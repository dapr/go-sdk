...
func (s *Server) OnInvoke(ctx context.Context, in *connect.Request[commonv1.InvokeRequest]) (*connect.Response[commonv1.InvokeResponse], error) {
	if in == nil {
	...
	if fn, ok := s.invokeHandlers[in.Msg.GetMethod()]; ok {
		e := &cc.InvocationEvent{}
		e.ContentType = in.Msg.GetContentType()

		if in.Msg.GetData() != nil {
			e.Data = in.Msg.GetData().GetValue()
			e.DataTypeURL = in.Msg.GetData().GetTypeUrl()
		}

		if in.Msg.GetHttpExtension() != nil {
			e.Verb = in.Msg.GetHttpExtension().GetVerb().String()
			e.QueryString = in.Msg.GetHttpExtension().GetQuerystring()
		}
		...
		if ct == nil {
			return connect.NewResponse(&commonv1.InvokeResponse{}), nil
		}

		return connect.NewResponse(&commonv1.InvokeResponse{
	...
	return nil, fmt.Errorf("method not implemented: %s", in.Msg.GetMethod())
}
