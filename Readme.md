...
To publish data onto a topic the Dapr client provides a simple method requiring the name of the pub/sub component, name of the topic, and the data:

```go
data := []byte(`{ "id": "a123", "value": "abcdefg", "valid": true }`)
if err := client.PublishEvent(ctx, "messagebus", "topic-name", data); err != nil {
    panic(err)
}
```
